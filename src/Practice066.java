...
class BinarySearchTree {
    ...
    public Node addNode(Node cur, int key) {
        if(cur == null) { // 만약 현재위치에 아무것도 없으면
            return new Node(key, null, null); // 새 노드 추가
        }
        // 키 값이 현재 키 값보다 작으면
        if(key < cur.key) {
...
public class Practice066 {
    ...
    public static void inOrder(Node node, ArrayList list) {
        ...
        inOrder(node.left, list); // 가장 왼쪽 리프노드까지 가서
        list.add(node.key); // 노드의 키 값 리스트에 추가
        inOrder(node.right, list); // 그 다음 오른쪽 리프노드로 넘어가기
    }
